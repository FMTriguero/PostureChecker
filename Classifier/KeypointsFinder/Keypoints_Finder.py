...
class KeypointsFinder:
    ...
    def test_show_image_prepare_nodes(self):
        canvas = self.working_image
        for i in range(18):
            for j in range(len(self.working_all_peaks[i])):
                cv2.circle(canvas, self.working_all_peaks[i][j][0:2], 4, self.colors[i], thickness=-1)

    def test_show_image(self):
        self.test_show_image_prepare_nodes()
        # sometimes opencv will oversize the image when using using `cv2.imshow()`. This function solves that issue.
        screen_res = 1280, 720
        scale_width = screen_res[0] / self.working_image.shape[1]
        scale_height = screen_res[1] / self.working_image.shape[0]
        scale = min(scale_width, scale_height)
        window_width = int(self.working_image.shape[1] * scale)
        window_height = int(self.working_image.shape[0] * scale)
        cv2.namedWindow('image', cv2.WINDOW_NORMAL)
        cv2.resizeWindow('image', window_width, window_height)
        cv2.imshow('image', self.working_image)
        cv2.waitKey(0)
